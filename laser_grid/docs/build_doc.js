...
body.push(
  ...
  new Paragraph({ spacing: { after: 0 }, alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "2026-08-25 기준  ·  Python 17개 파일, 8,472줄",
...
body.push(...Code([
  "N_VERTICAL / N_HORIZONTAL = 21 / 21   # design",
  "DOE_PROJECTION_MM         = 936.0     # spec  120cm 에서 936x936mm",
  "",
  "fov_deg = 2·atan(468/1200) = 42.61°   (유도)",
]));
body.push(P("발사각은 이 발산각을 21등분해 생성한다. 실제로 계산되는 값은 다음과 같다.", { before: 100 }));
body.push(...Code([
  "α0 =-21.305°  α1 =-19.174°  α2 =-17.044°  α3 =-14.913°",
  "α4 =-12.783°  α5 =-10.652°  α6 = -8.522°  α7 = -6.391°",
  "α8 = -4.261°  α9 = -2.130°  α10=  0.000°  α11=  2.130°",
  "α12=  4.261°  α13=  6.391°  α14=  8.522°  α15= 10.652°",
  "α16= 12.783°  α17= 14.913°  α18= 17.044°  α19= 19.174°",
  "α20= 21.305°",
  "",
  "간격 일정 2.1305°  (등각도 가정).  β_j 도 동일한 값",
]));
...
body.push(H3("사양 충돌 — 격자가 시야보다 넓다"));
body.push(P("PDF 의 카메라 사양과 DOE 사양이 서로 맞지 않는다. calibration.check_consistency() 가 이를 검사해 경고한다."));
body.push(Gap(60));
body.push(Tbl(
  ["항목", "값", "1.2m 기준"],
  [
    ["카메라 HFOV (렌즈 12mm)", "38.77°", "시야 845 mm"],
    ["DOE 발산각 (투사 936mm)", { t: "42.61°", c: C.warn }, { t: "투사 936 mm", c: C.warn }],
    ["결과", { t: "격자가 시야보다 10% 넓다", c: C.warn },
     { t: "V선 21개 중 19개만 센서 안", c: C.warn }],
  ],
  [3400, 3200, 3038]));
body.push(P("해소안은 두 가지다. ① DOE 발산각을 37.30° 이하로 줄이면 21선이 모두 담기며 1.2m 투사폭은 810mm가 된다. ② 렌즈를 10.39mm 이하로 바꾸면 936mm 를 전부 수용한다. 현재 코드는 사양값을 그대로 넣고 경고만 띄운다. 격자가 시야를 벗어나는 것 자체가 현실이므로 임의로 숨기지 않았다.", { before: 140 }));
...
body.push(P("시뮬레이션 튜닝값을 실제 하드웨어 사양 기준으로 교체하였다."));
body.push(Gap(60));
body.push(Tbl(
  ["항목", "이전", "현재", "근거"],
  [
    ...
    ["DOE 발산각", { t: "60.82°", c: C.ink3 }, { t: "42.61°", c: C.accent },
     "투사폭 936mm @1.2m 에서 역산"],
    ["격자 선 수", "21 + 21", "21 + 21", "유지 (PDF 표기는 20+20)"],
    ["기선 b", "0.150 m", "0.150 m", "유지 (설계 고정값)"],
    ["주점 c_x, c_y", "1224, 1024", "1224, 1024", "유지 (센서 중앙 가정)"],
  ],
  [1800, 1500, 1500, 4838]));
body.push(Note("이 변경으로 시야가 좁아지면서(HFOV 75.1° → 38.77°) 촬영 자세도 바뀌었다. 기존 22° 하향으로는 바닥이 화면에 들어오지 않아(바닥 점 0개) 34° 로 조정하였다. 다만 더 눕힐수록 벽을 사각으로 보게 되어 평활도 분해능이 나빠지므로, 한 장으로 벽 평활도와 바닥 수평도를 동시에 최적화할 수는 없다."));
...
body.push(P("벽 + 바닥 + 동바리가 한 프레임에 들어오는 합성 씬에서 두 세그멘테이션 백엔드 모두 전 항목 통과하였다. 실제 렌즈 기준 시야가 좁아 장비를 34° 하향으로 잡았고, 그 결과 격자점은 2,980개(벽 2,372 · 바닥 345 · 동바리 263)다."));
body.push(Gap(60));
body.push(Tbl(
  ["부재", "검측 항목", "정답", "gt 백엔드", "geom 백엔드"],
  [
    ["벽", "수직도", "0.50 °", "오차 0.0052 °", "오차 0.0138 °"],
    ["바닥", "수평도", "0.30 °", "오차 0.0010 °", "오차 0.0301 °"],
    ["동바리", "축 수직도", "1.20 °", "오차 0.2511 °", "오차 0.2901 °"],
    ["벽", "요철 깊이", "융기 6.0 mm", "5.82 mm", "5.79 mm"],
    ["벽", "직선자 처짐", "—", "4.09 mm (하한)", "3.74 mm (하한)"],
  ],
...
body.push(Tbl(
  ["조건", "벽", "바닥", "동바리", "판정"],
  [
    ["정답 마스크 (기준선)", "0.0052", "0.0010", "0.2511", "통과"],
    ["마스크 −8px 침식", "0.0048", "0.0004", "0.2242", "통과"],
    ["마스크 +8px 팽창", "0.0052", "0.0002", "0.0876", "통과"],
    ["마스크 +16px 팽창", "0.0052", "0.0002", "0.2865", "통과"],
    ["라벨 오분류 100% (9종 전부)", "복구", "복구", "복구", "통과"],
    ["기하 전용 백엔드 (geom)", "0.0138", "0.0301", "0.2901", "통과"],
    ["σ_u = 1.0 px", "0.0028", "0.1955", "0.2735", "통과"],
  ],
...
body.push(Tbl(
  ...
  [
    ...
    ["사양 충돌", "카메라 HFOV 38.77° 보다 DOE 발산각 42.61° 가 넓어 V선 21개 중 2개가 센서를 벗어난다. DOE 를 37.30° 이하로 줄이거나 렌즈를 10.39mm 이하로 바꿔야 해소된다."],
