...
body.push(Tbl(
  ["데이터", "기호", "현재 값", "출처", "비고"],
  [
    ["카메라 초점거리", M("f"), M("3478.3 px"), "spec",
     "렌즈 12mm ÷ 화소 3.45µm 에서 유도." + SEP + "체커보드 캘리브레이션으로 검증 필요"],
    ["주점", M("c_x, c_y"), M("1224.0, 1024.0"), { t: "assumed", c: C.warn },
     { t: "센서 정중앙 가정. 실제 주점은" + SEP + "제조 공차로 수십 px 어긋난다", c: C.warn }],
    ["기선", M("b"), M("0.150 m"), "design",
     "용역서 고정값. 조립 후 실측 필요"],
    ["DOE 발산각", "—", M("31.0°"), "design",
     "21선이 작업거리 1.0~1.5m 내내" + SEP + "센서 안에 들어오는 최대값"],
    ["레이저 수렴각", M("δ"), M("5.1°"), "design",
     "격자를 센서 중앙에 오게 하는 설계값." + SEP + "시차 이동량을 상쇄한다"],
    ["V선 발사각", M("α_i"), M("등각도 21분할"), { t: "assumed", c: C.warn },
     { t: "DOE 회절은 등각도가 아니라 등 sin(각)" + SEP + "에 가깝다. 실측 대체 필요", c: C.warn }],
    ["H선 발사각", M("β_j"), M("등각도 21분할"), { t: "assumed", c: C.warn },
     { t: "동일", c: C.warn }],
...
body.push(Note("실측에 기반한 값은 기선 b=150mm 하나뿐이고, f 와 DOE 발산각은 부품 사양에서 유도한 값이다. 나머지 여섯 항목은 가정값이다. 이 여섯이 남아 있는 한 절대 정확도를 보증할 수 없으며, 현재 검증 결과는 「가정이 맞다면」이라는 단서 아래의 값이다."));
...
body.push(...Code([
  ...
  "PIXEL_PITCH_UM = 3.45      # spec   Sony IMX264",
  "IMAGE_W/H      = 2448/2048 # spec   5MP",
  "BASELINE_M     = 0.150     # design 용역서 고정",
  "",
  "f_px  = 12.0mm / 3.45um    = 3478.3 px   (유도)",
  "cx,cy = 2448/2, 2048/2     = 1224.0, 1024.0   (가정 — 센서 중앙)",
  "",
  "WORK_Z_MIN/MAX_M = 1.0 / 1.5   # spec   PDF 권장 측정거리",
  "EDGE_MARGIN_PX   = 50          # design 센서 가장자리 여유",
]));
body.push(P("카메라 HFOV 38.77°, VFOV 32.81°. 거리별 시야와 깊이 노이즈는 다음과 같다.", { before: 120 }));
body.push(Gap(60));
body.push(Tbl(
  ["거리", "시야 (가로 × 세로)", "분해능", "σ_Z (구 f=1593)", "σ_Z (신 f=3478)"],
  [
    ["0.5 m", "352 × 294 mm", "0.144 mm/px", "0.21 mm", { t: "0.10 mm", c: C.accent }],
    ["1.0 m", "704 × 589 mm", "0.288 mm/px", "0.84 mm", { t: "0.38 mm", c: C.accent }],
    ["1.2 m", "845 × 707 mm", "0.345 mm/px", "1.21 mm", { t: "0.55 mm", c: C.accent }],
    ["1.5 m", "1056 × 883 mm", "0.431 mm/px", "1.88 mm", { t: "0.86 mm", c: C.accent }],
    ["2.0 m", "1408 × 1178 mm", "0.575 mm/px", "3.35 mm", { t: "1.53 mm", c: C.accent }],
  ],
  [1000, 2600, 1700, 2100, 2238]));
body.push(Note("f_px 를 시뮬 튜닝값 1593 에서 사양 유도값 3478.3 으로 바꾸면서 깊이 노이즈가 절반 이하로 줄었다. PDF 5.2 는 「기선/거리 조합만으로는 σ_Z ≤ 2mm 목표 달성 불가」라고 결론냈으나, 이는 f=1593 기준이다. 실제 12mm 렌즈에서는 1.5m 까지 0.86mm 로 목표를 만족한다. 렌즈 선정만으로 결론이 달라지는 부분이라 확인이 필요하다."));
...
body.push(...Code([
  ...
  "",
  "  시차 이동량 f·b/Z",
  "    Z=0.5m → 1044 px  (센서 폭의 42.6%)",
  "    Z=1.0m →  522 px  (21.3%)",
  "    Z=1.2m →  435 px  (17.8%)",
  "    Z=1.5m →  348 px  (14.2%)",
]));
body.push(P("이 이동량 때문에 레이저 축을 카메라와 평행하게 두면 센서 한쪽이 통째로 낭비된다. 레이저를 카메라 쪽으로 조금 기울이면 격자가 작업거리에서 화면 중앙에 오므로 양쪽을 고르게 쓸 수 있다.", { before: 120 }));
body.push(Gap(60));
body.push(Tbl(
  ["레이저 축 배치", "담을 수 있는 발산각", "1.2m 투사폭", "비고"],
  [
    ["카메라와 평행 (δ = 0)", { t: "21.24°", c: C.warn }, { t: "450 mm", c: C.warn },
     "센서 한쪽이 시차 이동량만큼 낭비"],
    ["5.1° 수렴 (채택)", { t: "31.0°", c: C.accent }, { t: "666 mm", c: C.accent },
     "1.47배. 세로 시야가 제약이 된다"],
    ["PDF 사양 (936mm @1.2m)", "42.61°", "936 mm",
     "12mm 렌즈 HFOV 38.77° 초과 — 담기지 않음"],
  ],
  [2400, 2000, 1600, 3638]));
body.push(P("삼각측량 기하는 그대로이고 발사각의 기준축만 바뀌므로, α_i 에 수렴각을 더해 쓰면 된다(탄젠트 덧셈정리로 정확히 성립). β 는 1/(cosδ − sinδ·tanα) 만큼 살짝 결합되어 격자가 미세한 사다리꼴이 되지만, 깊이 Z 는 α 와 u 로만 정해지므로 영향이 없고 H선 예측 위치에만 최대 24px 반영된다.", { before: 140 }));

body.push(H3("정합성 검사 결과"));
body.push(...Code([
  "작업거리 1.0 ~ 1.5 m,  마진 50px",
  "  Z=1.0m   u =   63.9 .. 2009.7    (허용 50 .. 2398)",
  "  Z=1.5m   u =  237.8 .. 2183.6",
  "  v      =   59.4 .. 1988.6        (허용 50 .. 1998)",
  "",
  "  판정: 격자 전부 센서 안",
  "  이 설계로 쓸 수 있는 거리: 0.97 ~ 3.91 m",
]));
body.push(Note("PDF 2.2 의 「120cm 에서 936×936mm」는 발산각 42.61° 에 해당하는데, 12mm 렌즈의 HFOV 38.77° 보다 넓어 그대로는 담기지 않는다. 936mm 를 유지하려면 렌즈를 10.4mm 이하로 낮춰야 한다. 현재 설계는 렌즈를 12mm 로 유지하고 발산각을 31.0° 로 맞췄다."));

body.push(H3("발사각"));
body.push(P("발산각 31.0° 를 21등분한 뒤 수렴각 5.1° 를 더한 값이다."));
body.push(...Code([
  "α0 =-10.400°  α1 = -8.850°  α2 = -7.300°  α3 = -5.750°",
  "α4 = -4.200°  α5 = -2.650°  α6 = -1.100°  α7 =  0.450°",
  "α8 =  2.000°  α9 =  3.550°  α10=  5.100°  α11=  6.650°",
  "α12=  8.200°  α13=  9.750°  α14= 11.300°  α15= 12.850°",
  "α16= 14.400°  α17= 15.950°  α18= 17.500°  α19= 19.050°",
  "α20= 20.600°",
  "",
  "간격 일정 1.5500°.  β_j 는 수렴각 없이 -15.5° .. +15.5°",
]));
...
body.push(H2("5.5 값 변경 내역"));
body.push(Tbl(
  ["항목", "이전", "현재", "근거"],
  [
    ["초점거리 f_px", { t: "1593.0", c: C.ink3 }, { t: "3478.3", c: C.accent },
     "렌즈 12mm ÷ 화소 3.45µm." + SEP + "1593 은 5.50mm 렌즈에 해당했다"],
    ["DOE 발산각", { t: "60.82°", c: C.ink3 }, { t: "31.0°", c: C.accent },
     "21선이 1.0~1.5m 내내 센서 안에" + SEP + "들어오는 최대값"],
    ["레이저 수렴각", { t: "없음", c: C.ink3 }, { t: "5.1°", c: C.accent },
     "시차 이동량 상쇄. 발산각을" + SEP + "21.24°에서 31.0°로 넓힌다"],
    ["격자 선 수", "21 + 21", "21 + 21", "유지"],
    ["기선 b", "0.150 m", "0.150 m", "유지 (설계 고정값)"],
    ["주점 c_x, c_y", "1224, 1024", "1224, 1024", "유지 (센서 중앙 가정)"],
  ],
  [1700, 1400, 1400, 5138]));

body.push(H3("기하 예측의 시차항 누락 수정"));
body.push(P("A_선검출의 격자 위치 예측이 u = f·tan(α) + c_x 로, 시차항 −f·b/Z 를 빼먹고 있었다. 예측은 추적 밴드의 중심을 잡는 데 쓰이는데 밴드 폭이 20~50px 이므로, 435px 어긋난 예측은 밴드를 실제 선 근처에도 놓지 못한다. 이전 f=1593 에서도 199px 어긋나 있었으므로 새로 생긴 문제가 아니다."));
body.push(Gap(60));
body.push(Tbl(
...
body.push(P("f_px 는 2448px 센서 기준값이다(당시 1593, 현재 3478.3). 471px 이미지에 그대로 쓰면 예측 격자가 화면 밖으로 나가 21개 중 5개만 안에 들어온다. 해상도에 맞춰 환산하면 21개 전부 화면 안에 들어오고 평균 오차가 332px에서 7px로 떨어진다. 다만 7px도 격자 간격 23px의 30%라 측정에는 쓸 수 없다. 이 환산은 calibration.scale_to_resolution() 으로 제공한다."));
body.push(H3("③ 조용한 실패"));
...
body.push(P("벽 + 바닥 + 동바리가 한 프레임에 들어오는 합성 씬에서 두 세그멘테이션 백엔드 모두 전 항목 통과하였다. 실제 렌즈 기준 시야가 좁아 장비를 34° 하향으로 잡았다. 격자가 전부 시야에 들어오도록 설계한 뒤 격자점은 4,965개(벽 4,003 · 바닥 491 · 동바리 471)다."));
body.push(Gap(60));
body.push(Tbl(
  ["부재", "검측 항목", "정답", "gt 백엔드", "geom 백엔드"],
  [
    ["벽", "수직도", "0.50 °", "오차 0.0085 °", "오차 0.0044 °"],
    ["바닥", "수평도", "0.30 °", "오차 0.0410 °", "오차 0.0400 °"],
    ["동바리", "축 수직도", "1.20 °", "오차 0.0229 °", "오차 0.0040 °"],
    ["벽", "요철 깊이", "융기 6.0 mm", "5.82 mm", "5.82 mm"],
    ["벽", "직선자 처짐", "—", "4.03 mm (하한)", "3.52 mm (하한)"],
  ],
  [1100, 1900, 1700, 2400, 2538]));
body.push(P("허용 기준은 ±0.5°이다. 직선자 처짐은 요철 폭이 측정 분해능보다 좁아 하한값으로 나오며, 시스템이 분해능 한계를 함께 보고한다.", { before: 100, size: 18, color: C.ink2 }));

body.push(H3("동바리 — 노출 길이가 정확도를 지배한다"));
body.push(P("실제 렌즈의 세로 시야는 1.2m 에서 707mm 로 좁다. 2.4m 부재 중 일부만 담기며, 짧게 담길수록 원통 단면이 주축을 끌어당겨 각도가 흔들린다. Ø48.6mm 파이프서포트로 측정한 값이다."));
body.push(Gap(60));
body.push(Tbl(
...
body.push(Tbl(
  ["조건", "벽", "바닥", "동바리", "판정"],
  [
    ["정답 마스크 (기준선)", "0.0085", "0.0410", "0.0229", "통과"],
    ["마스크 +2px 팽창", "0.0085", "0.0410", "0.0229", "통과"],
    ["마스크 +8px 팽창", "0.0085", "0.0410", "0.0229", "통과"],
    ["마스크 +16px 팽창", "0.0085", "0.0410", "0.0487", "통과"],
    ["라벨 오분류 9종 전부", "복구", "복구", "복구", "통과"],
    ["기하 전용 백엔드 (geom)", "0.0044", "0.0400", "0.0040", "통과"],
  ],
...
body.push(Tbl(
  ["항목", "내용"],
  [
    ["평활도 과소보고", "직선자 처짐이 정답의 60~70% 수준으로 나온다. 실제 렌즈의 좁은 시야 때문에 바닥을 담으려면 장비를 34° 이상 기울여야 하고, 그러면 벽을 사각으로 보게 되어 면내 점밀도가 낮아진다. σ=5cm 요철이 그 분해능 아래로 내려간다. 요철 깊이(eq4)는 정점 근방 원시잔차라 5.8mm 로 정답에 가깝다."],
    ...
    ["캘리브레이션 미확보", "4.2절 표에서 assumed 로 표시한 여섯 항목(c_x·c_y, α_i, β_j, R·t, R_ic, b_a)과 σ_u 가 가정값이다. f 와 DOE 발산각은 부품 사양에서 유도했고, 실측 기반은 기선 b 하나뿐이다."],
    ["DOE 사양 재검토 필요", "PDF 의 투사폭 936mm(42.61°)는 12mm 렌즈로 담기지 않아 31.0°(1.2m 투사 666mm)로 낮췄다. 936mm 를 유지하려면 렌즈를 10.4mm 이하로 바꿔야 한다. 레이저 축 5.1° 수렴은 기구 설계에 반영이 필요하다."],
