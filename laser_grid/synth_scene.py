...
#
# 의 최대값이다 (A=6mm, σ=50mm, D = 프로파일 반폭). 이 씬의 벽에서
# 자가 걸리는 반폭은 430mm 이고, 그때 최대 틈은 3.99mm 다.
#
# 무잡음·초고밀도(V선 250개) 극한에서 파이프라인이 내는 값도 3.99mm 로
# 해석값과 일치한다. 즉 알고리즘에 계통 편향이 없다. 이전 문서가
# "평활도를 정답의 60~70% 로 과소보고" 라고 적은 것은 6.0mm(융기 높이)와
# 비교한 탓이며, 실제로는 참값을 맞히고 있었다.
GT_STRAIGHTEDGE_MM = 3.99
